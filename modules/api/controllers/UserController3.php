    ...
    public function actionPostupload()
    {
        ...
        if(isset($attributes['description']) && !empty($attributes['description']))
        {
            ...
            if($object->save())
            {
                ...
                $entityid = \Yii::$app->db->getLastInsertID();
                //$tbentity = \Yii::$app->db->createCommand()->insert('tb_entities_metadata', ['guid' => $entityid,'value' => $attributes['userid']])->execute();
		if($attributes['type'] == 'group' && isset($attributes['createid']) && !empty($attributes['createid']))
                    $tbentity = \Yii::$app->db->createCommand()->insert('tb_entities_metadata', ['guid' => $entityid,'value' => $attributes['createid']])->execute();
                else
                    $tbentity = \Yii::$app->db->createCommand()->insert('tb_entities_metadata', ['guid' => $entityid,'value' => $attributes['userid']])->execute();
        ...
        if(isset($attributes['message']) && !empty($attributes['message']))
        {
            ...
            if($message->save(false)) {
            	if(isset($_FILES["imagepath"]["name"]) && !empty($_FILES["imagepath"]["name"]))
                {
                    ...
                    if (!file_exists('/var/sentora/hostdata/zadmin/public_html/thetalentbook_co_in/tb_data/message/'.$message->id))
                        mkdir('/var/sentora/hostdata/zadmin/public_html/thetalentbook_co_in/tb_data/message/'.$message->id, 0777, true);
                        
                    $extension = pathinfo($_FILES["imagepath"]["name"], PATHINFO_EXTENSION);
                    $name = substr(base_convert(sha1(uniqid(mt_rand())), 16, 36), 0, '8');
                    $target_file = '/var/sentora/hostdata/zadmin/public_html/thetalentbook_co_in/tb_data/message/'.$message->id.'/'.$name.".".$extension;
    ...
    public function actionGetallcredits()
    {
        ...
        if(!empty($credits))
        {
            foreach($credits as $key=>$value)
            {
                ...
                $profilephoto = '';
                $sql = "select tem.* from tb_entities te, tb_entities_metadata tem where te.guid=tem.guid and te.owner_guid=".$value['userid']." and te.type='user' and te.subtype='file:profile:photo'";
                $resultphoto = \Yii::$app->db->createCommand($sql)->queryOne();
                if(!empty($resultphoto))
