...
use app\modules\models\BaseObject;
...
class UserController extends \yii\web\Controller
{
        ...
        if(!empty($value))
        {
            ...
            if((!empty($description)))
            {
                ...
                $data = array('post_id'=>$value['guid'],'userid'=>$value['owner_guid'], 'type'=>$value['type'], 
                    ...
                    'mobile'=>$usercheck['mobile'], 'birthdate'=>$birthdate, 'gender'=>$gender, 
                    'profilephoto'=>$profilephoto));
                    // 'college'=>$usercheck['college'], 'location'=>$usercheck['location'], 
        ...
        if(count(array($user)) > 0)
        {
            ...
            if(count(array($usercheck)) > 0 )
            {
                ...
                $data = array('userid'=>$usercheck->guid, 'type'=>$usercheck->type, 'username'=>$usercheck->username, 'email'=>$usercheck->email,
                    'first_name'=>$usercheck->first_name, 'last_name'=>$usercheck->last_name, 'mobile'=>$usercheck->mobile,  'birthdate'=>$birthdate, 'gender'=>$gender, 'profilephoto'=>$profilephoto);//,'regtype'=>$usercheck->RegType,'regid'=>$usercheck->Regid);
    ...
    public function actionShareusers()
    {
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        $attributes = \yii::$app->request->get();
        $postshare = Shares::find()->where(['subject_id'=>$attributes['postid']])->all();
        if(count($postshare) > 0 )
        {
            $data = '';
            foreach($postshare as $key=>$value)
            {
                $usercheck = Users::find()->where(['guid' => $value['guid']])->one();
                if(count($usercheck) > 0)
                {
    ...
    public function actionCommentcreate()
    {
        ...
        if(isset($attributes['postid']) && !empty($attributes['postid']))
        {
            ...
            $usercheck = Users::find()->where(['guid' => $attributes['userid']])->one();
            $commentinfo = array('commentid'=>$annotationid, 'userid'=>$usercheck['guid'], 'type'=>$usercheck['type'], 'email'=>$usercheck['email'],
                        'username'=>$usercheck['username'], 'mobile'=>$usercheck['mobile'], 'first_name'=>$usercheck['first_name'], 
                        'last_name'=>$usercheck['last_name'],'description'=>$usercheck['description'],'location'=>$usercheck['location']);
        ...
        if(count($comments) > 0 )
        {
            ...
            foreach($comments as $key=>$value)
            {
                ...
                $commentinfo = array('commentid'=>$value['tid'],'comment'=>$comentmessage, 'userid'=>$usercheck['guid'], 'type'=>$usercheck['type'], 
                        'username'=>$usercheck['username'], 'email'=>$usercheck['email'], 'first_name'=>$usercheck['first_name'], 'last_name'=>$usercheck['last_name'],
                         'mobile'=>$usercheck['mobile'], 'location'=>$usercheck['location'], 'description'=>$usercheck['description'],
                         'likestatus'=>$likestatus, 'likecount'=>$commentcount);
            }
        ...
        if(count($relations) > 0 )
        {
            ...
            foreach($relations as $key=>$value)
            {
                ...
                $friends = array('userid'=>$usercheck['guid'], 'type'=>$usercheck['type'], 
                            'username'=>$usercheck['username'], 'email'=>$usercheck['email'], 'first_name'=>$usercheck['first_name'], 'last_name'=>$usercheck['last_name'], 
                            'mobile'=>$usercheck['mobile'],'location'=>$usercheck['location'], 
...
//             var_dump($newobject->guid);
    ...
    public function actionRegister()
    {
       ...
       //$user->student=isset($attributes['student']) ? $attributes['student'] : '' ;
        //$user->school_teacher=isset($attributes['school_teacher']) ? $attributes['school_teacher'] : '' ;
        //$user->professional=isset($attributes['professional']) ? $attributes['professional'] : '' ;
        //$user->school_management=isset($attributes['school_management']) ? $attributes['school_management'] : '' ;
        ...
        {
            if ($attributes['usertype'] == 'mobile') {
                $btext = '<p>Before you can start using Talentbook, you must confirm your phone number.</p>';
    ...
    public function actionLogin(){
        ...
        if(isset($username) && isset($password)){
            ...
            //fetch the salt of respective username
            $saltsql = "select salt from k12testdata where username = '".$username."'";
            $command = \Yii::$app->db->createCommand($saltsql);
            $row = $command->queryOne();
            $salt = $row['salt'];
            //assign salt encoded password to $password
            $password = md5($password.$salt);
            //verify password
            $sql = "select username,password,salt from k12testdata where username = '".$username."' AND password = '".$password."'";
            $rows= \Yii::$app->db->createCommand($sql);
            $dataReader= $rows->query();
            $row_count = $dataReader->rowCount;
            ...
            if ($row_count == 1) {
                ...
                return array('status'=>1, 'msg'=>'Login successful!', 'error'=>'');
            }else{
                return array('status'=>0, 'error'=>'Username or password invalid');
            }
        }
        else{
            return array('status'=>0, 'error'=>'');
